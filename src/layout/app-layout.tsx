...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['app-owner', 'app-administrator', 'app-developer'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<OwnerInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/owners` : null),
    () => getOwners({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Owners',
      path: isTenantUser ? `/owners/view/${data?.[0]?.id}` : '/owners',
      entity: 'owner',
