  ...
  const fetcher = useCallback(
    async () =>
      getApps({
        ...
        order: params.order,
        searchTermKeys: ['name.contains', 'version.contains', 'status.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<AppInterface>>(
    () => `/apps?params=${JSON.stringify(params)}`,
    fetcher,
  );
    ...
    hasAccess('owner', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/owners/view/${record.owner?.id}`}>
              {record.owner?.name}
            </Link>
          ),
        }
